...
Local packaging control plane for Joushen Studio.
...
Release artifacts keep the canonical `Joushen Studio.app` bundle shape; local `tools-pack install` copies it as
`Joushen Studio.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
...
- Finder/manual launches on macOS may not inherit your shell-managed `PATH`. If packaged Joushen Studio cannot detect agent CLIs that work in Terminal, expose those binaries to the GUI login environment or launch the packaged app from a shell session that already sees them.
...
`--headless` makes `install`, `start`, `stop`, `uninstall`, and `cleanup` operate on the headless entry (`@joushen-studio/packaged/dist/headless.mjs`) instead of the AppImage. Headless mode runs daemon + web without Electron.
