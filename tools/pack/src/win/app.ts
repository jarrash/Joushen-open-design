...
import { createCommandInvocation, createPackageManagerInvocation } from "@joushen-studio/platform";
...
async function runEsbuild(config: ToolPackConfig, args: string[]): Promise<void> {
  await runPnpm(config, ["--filter", "@joushen-studio/packaged", "exec", "esbuild", ...args]);
}
...
async function buildWorkspaceArtifacts(config: ToolPackConfig): Promise<void> {
  ...
  try {
    await runPnpm(config, ["--filter", "@joushen-studio/web", "build"], { OD_WEB_OUTPUT_MODE: config.webOutputMode });
    ...
    `${JSON.stringify(
      {
        ...
        description: "Joushen Studio packaged runtime",
        main: "./main.cjs",
