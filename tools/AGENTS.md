...
- `tools/dev` provides `@joushen-studio/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@joushen-studio/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset plus beta release artifact preparation for mac and Windows lanes, plus a Linux AppImage lane with optional containerized builds.
- `tools/serve` provides `@joushen-studio/tools-serve` and the `tools-serve` bin. It owns local fixture services such as `tools-serve start updater`.

## Retired tools

- `tools/pr` / `@joushen-studio/tools-pr` / `pnpm tools-pr` has been retired from this repository. Maintainer PR-duty workflows now live outside the product workspace in `PerishCode/duty`; do not restore a Joushen Studio-local PR-duty tool without a new explicit maintainer decision.
...
- Orchestration layers must consume primitives from `@joushen-studio/sidecar-proto`, `@joushen-studio/sidecar`, and `@joushen-studio/platform`.
...
```bash
pnpm --filter @joushen-studio/tools-dev typecheck
