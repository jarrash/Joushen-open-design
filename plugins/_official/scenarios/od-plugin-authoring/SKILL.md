...
description: Guided scenario for creating a Joushen Studio plugin folder that can be installed into My plugins.
od:
...
Use this scenario when the user wants to create their own Joushen Studio plugin.
...
- Treat `SKILL.md` as the canonical behavior description. `open-design.json` should describe how Joushen Studio installs, applies, and presents that behavior.
...
- Include a short readiness summary when finished:
  ...
  - A direct next-action prompt that offers: Add to My plugins, Publish repo, or Joushen Studio PR.
