...
description: >
  ...
  collage) — the same aesthetic Joushen Studio uses for its own marketing
...
This is the canonical Joushen Studio marketing-page recipe — the example
...
worked example (Joushen Studio itself).
...
`pnpm --filter @joushen-studio/landing-page build` for a static `out/`
...
├── inputs.example.json      # Joushen Studio as the worked example
...
└── assets/
    ├── *.png                # 16 collage plates (Joushen Studio instance)
