...
- [ ] **Run `pnpm --filter @joushen-studio/web test` and `pnpm --filter @joushen-studio/e2e test tests/localized-content.test.ts`** locally before pushing. These suites catch undisplayable discovered resources and verify localized fallback behavior.
...
- [ ] Ran `pnpm --filter @joushen-studio/web test` and `pnpm --filter @joushen-studio/e2e test tests/localized-content.test.ts`; localized-content coverage is green
