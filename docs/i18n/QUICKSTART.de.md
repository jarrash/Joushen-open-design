...
pnpm --filter @joushen-studio/daemon build  # apps/daemon/dist/cli.js für `od` bauen
pnpm --filter @joushen-studio/web build     # Web-Paket bei Bedarf bauen
...
Führen Sie Joushen Studio in einer vollständig containerisierten Umgebung aus, ohne Node.js oder pnpm lokal zu installieren.
...
Öffnen Sie danach das Projekt erneut aus der Joushen Studio App, statt eine alte Terminal-Agent-Session fortzusetzen. Ein vom daemon gestarteter Agent sollte Werte wie diese sehen:
...
│   ├── sidecar-proto/         # Joushen Studio sidecar protocol contract
...
- **media generation says `OD_BIN` is missing or daemon URL is `:0`** — führen Sie die Media Dispatcher Checks oben aus. Setzen Sie keine alte CLI-Session fort; öffnen Sie das Projekt aus der Joushen Studio App neu, damit der daemon frische `OD_*` Variablen injiziert.
- **Codex lädt zu viel Plugin-Kontext** — starten Sie Joushen Studio mit `OD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev`, damit vom daemon gestartete Codex-Prozesse mit `--disable plugins` laufen.
