...
This PR wires Joushen Studio's beta mac arm64 packaging path to Vela's npm-owned CLI distribution contract.
...
Joushen Studio depends only on the Vela meta package:

```json
"@powerformer/vela-cli": "0.0.1-test"
```

Joushen Studio does not depend directly on platform packages such as `@powerformer/vela-cli-darwin-arm64`. Vela owns that platform matrix through optional dependencies and its resolver API.
...
1. Vela publishes `@powerformer/vela-cli` to npm before Joushen Studio packaging starts.
2. Joushen Studio pins that package in `tools/pack/package.json` and `pnpm-lock.yaml`.
...
4. `tools-pack` enters the `resource-tree` phase and resolves/copies the Vela binary into the Joushen Studio resource tree.
...
Vela resource copying now lives in `tools/pack/src/vela-cli.ts`, so the generic resource-tree helper only owns static Joushen Studio resources. The Vela helper resolves the binary through the shared resolver path and copies it into:
...
Joushen Studio should install only `@powerformer/vela-cli`. The meta package pulls the macOS arm64 binary package as an optional dependency on supported machines.
...
- Joushen Studio depends only on `@powerformer/vela-cli`.
