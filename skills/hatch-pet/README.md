...
checked into the Joushen Studio repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Joushen Studio agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Joushen Studio and
...
2. Replace the contents of this directory with the upstream snapshot,
   ...
   inside `SKILL.md`'s `> **Joushen Studio integration.**` blockquote.
...
flow (including how Joushen Studio behaves when Codex is not installed).
