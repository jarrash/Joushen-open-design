...
- `packages/sidecar-proto`: Joushen Studio sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, IPC message schema, status shapes, error semantics, and default product path constants.
- `packages/sidecar`: generic sidecar runtime primitives. Includes bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime file helpers; it must not hard-code Joushen Studio app keys or IPC business messages.
...
- Do not hard-code Joushen Studio app/source/mode constants in `sidecar` or `platform`.
- Keep stamp fields limited to five: `app`, `mode`, `namespace`, `ipc`, and `source`.

## Common package commands

```bash
pnpm --filter @joushen-studio/contracts typecheck
pnpm --filter @joushen-studio/host typecheck
pnpm --filter @joushen-studio/host test
pnpm --filter @joushen-studio/metatool typecheck
pnpm --filter @joushen-studio/metatool test
pnpm --filter @joushen-studio/sidecar-proto typecheck
pnpm --filter @joushen-studio/sidecar-proto test
pnpm --filter @joushen-studio/sidecar typecheck
pnpm --filter @joushen-studio/sidecar test
pnpm --filter @joushen-studio/platform typecheck
pnpm --filter @joushen-studio/platform test
```
