...
import { APP_KEYS, normalizeNamespace } from "@joushen-studio/sidecar-proto";
  ...
  if (odDataDir) {
    ...
    if (!isAbs) {
      throw new PackagedPathAccessError(
        [
          "Joushen Studio's packaged runtime requires OD_DATA_DIR to be an absolute path.",
          "",
          `Configured value: ${odDataDir}`,
          "",
          "Set OD_DATA_DIR to an absolute path (for example, C:\\\\Users\\\\You\\\\JoushenStudio on Windows or /Users/you/JoushenStudio on macOS/Linux) and relaunch Joushen Studio.",
        ].join("\n"),
        { title: "Joushen Studio cannot start with this OD_DATA_DIR" },
      );
    }
    ...
    if (scopedNamespace) {
      if (scopedNamespace !== namespace) {
        throw new PackagedPathAccessError(
          [
            "Joushen Studio's packaged runtime requires OD_DATA_DIR to target the active namespace.",
            "",
            `Configured value: ${odDataDir}`,
...
export function resolvePackagedNamespacePaths(
  ...
  // `~/Library/Application Support/Joushen Studio Nightly/` on mac. Custom
