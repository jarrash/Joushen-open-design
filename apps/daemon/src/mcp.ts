...
// (Claude Code, Cursor, Zed) pull files from a local Joushen Studio
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no Joushen Studio activity).',
} as const;

const TOOL_DEFS = [
  {
    name: 'list_projects',
    description: 'List every Joushen Studio project on this daemon.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Joushen Studio projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in Joushen Studio right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with Joushen Studio (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no Joushen Studio activity.",
        },
        include: {
          type: 'string',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Get Joushen Studio project' },
  },
  {
    name: 'get_file',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no Joushen Studio activity.',
        },
        offset: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Create one normal Joushen Studio project artifact entry file. Writes name+content, rejects existing targets, and persists artifactManifest when supplied. HTML, Markdown, and SVG entries get a default manifest when omitted. Project optional; defaults to the active project.',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        artifactManifest: {
          ...
          description: 'Optional ArtifactManifest sidecar. If omitted, Joushen Studio infers one for HTML, Markdown, or SVG entry files.',
        },
      },
      required: ['name', 'content'],
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Joushen Studio artifact' },
  },
  {
    name: 'write_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Write Joushen Studio project file' },
  },
  {
    name: 'delete_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete Joushen Studio project file' },
  },
  {
    name: 'delete_project',
    description:
      'Permanently delete a Joushen Studio project including its files and conversations. Requires both an explicit project id/name AND confirm:true — there is no active-project fallback because the operation is irreversible.',
    inputSchema: {
      type: 'object',
      properties: {
        project: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete Joushen Studio project' },
  },
  {
    name: 'create_project',
    description:
      'Create a new empty Joushen Studio project to generate into, then call start_run against it. Returns the project (with its id) plus a conversationId. The id is derived from name unless you pass one explicitly.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Joushen Studio project' },
  },
  // Discovery + generation. An external coding agent does NOT run a
  // skill itself — it commissions Joushen Studio to, via start_run. The
  ...
  {
    name: 'list_skills',
    description: 'List Joushen Studio skills you can pass to start_run as a recipe. Discovery only — Joushen Studio runs the skill, not you.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Joushen Studio skills' },
  },
  {
    name: 'list_plugins',
    description: 'List installed Joushen Studio plugins (packaged design workflows) you can pass to start_run as plugin + inputs.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Joushen Studio plugins' },
  },
  {
    name: 'start_run',
    description:
      'Commission Joushen Studio to generate or refine a design. Joushen Studio spawns its own agent to do the work and returns a runId immediately. Poll get_run(runId) until status is terminal, then get_artifact to pull the result. Project optional; defaults to the active project. Requires an existing project (create one first with create_project).',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        agent: {
          type: 'string',
          description: "Which agent Joushen Studio should run, e.g. 'claude' | 'codex' | 'gemini'. Optional; defaults to the user's configured agent.",
        },
        model: {
          type: 'string',
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Generate with Joushen Studio' },
  },
  {
    name: 'get_run',
    description:
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Check Joushen Studio run' },
  },
  {
    name: 'cancel_run',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'List Joushen Studio agents' },
  },
];
...
export async function runMcpStdio({ daemonUrl }: RunMcpOptions): Promise<void> {
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        'Joushen Studio (OD) is a local-first design workspace. The user typically',
        ...
        '',
        'To make Joushen Studio GENERATE or refine a design (rather than just',
        ...
        '    Joushen Studio spawns its own agent to do the work.',
        ...
        '',
        'Generation patience: Joushen Studio runs typically take 5–30',
        ...
        'deliverables — Joushen Studio natively produces browser-viewable',
        'HTML/SVG (including HTML-rendered decks), but the user may want a',
        'real binary file (.pptx / .docx / .pdf) which Joushen Studio does NOT',
        ...
        '',
        'When extending a Joushen Studio design in another codebase, pull',
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => {
    ...
    const resources = [
      {
        uri: 'od://focus/active',
        name: 'Active Joushen Studio context',
        description: 'The project/file the user has open in Joushen Studio right now.',
    ...
    switch (name) {
      ...
      case 'get_active_context': {
        ...
        if (!data || data.active === false) {
          return ok({
            active: false,
            hint: 'Joushen Studio has no active project right now. The active context expires about 5 minutes after the last user interaction with Joushen Studio, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
          });
        }
        return ok(data);
      }
...
// the user had already created in Joushen Studio.
//
...
async function getRun(baseUrl: string, args: McpArgs) {
  ...
  if (status.status !== 'succeeded' || typeof status.projectId !== 'string' || !status.projectId) {
    ...
    if (typeof status.eventsLogPath === 'string' && status.eventsLogPath.length > 0) {
      ...
      if (studioUrl) {
        enriched.hint += ` Once you have something to show the user, give them a clickable markdown link to studioUrl — render it as \`[Watch progress in Joushen Studio studio](${studioUrl})\`, NEVER as inline code or bare text, so clients like Codex / Cursor / Zed make it navigable in their built-in browser pane.`;
      }
    }
    return ok(enriched);
  }
...
// host-injected React/Babel do not — those still need the Joushen Studio
...
// renamed in the Joushen Studio UI shows up within a few seconds.
...
// open in Joushen Studio. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(baseUrl: string, arg: unknown): Promise<{ id: string; resolved: ResolvedProject | null; active: ActiveContext | null }> {
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and Joushen Studio has no active project. The active context expires about 5 minutes after the last user interaction with Joushen Studio - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the Joushen Studio daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
