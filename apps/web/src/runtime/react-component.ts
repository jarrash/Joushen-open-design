    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__JoushenStudioComponent ||
...
window.__JoushenStudioComponent = window.__JoushenStudioComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'JoushenStudioComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'JoushenStudioComponent';
    return 'const JoushenStudioComponent = ';
  });
  code = code.replace(
