...
//
// Joushen Studio connects to the configured servers as a CLIENT and surfaces
...
import type { TrackingExternalMcpElement } from '@joushen-studio/contracts/analytics';
import {
  disconnectMcpOAuth,
  fetchMcpOAuthStatus,
  fetchMcpServers,
...
// so the most useful categories for Joushen Studio (visual generation, then
      ...
      {expanded ? (
        <>
          ...
          {isHttpLike && !row._isNew && row.id ? (
            usesManagedOAuth ? (
              ...
              <div className="mcp-oauth-hint hint">
                <strong>No managed OAuth.</strong> Joushen Studio will use this
          ...
          {isHttpLike && row._isNew && usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant Joushen Studio
              access via the provider's OAuth flow.
            </div>
          ) : null}
          {isHttpLike && row._isNew && !usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              <strong>No managed OAuth.</strong> Save this server and Joushen Studio
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant Joushen Studio access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
