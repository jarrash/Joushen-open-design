...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Joushen Studio community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Joushen Studio CLI workflow from the current project workspace:',
    '',
...
// Joushen Studio issue URL and belongs to the "Joushen Studio PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits a Joushen Studio issue URL and belongs to the "Joushen Studio PR" button.',
    '',
    'Run this deterministic Joushen Studio CLI workflow from the current project workspace:',
    '',
