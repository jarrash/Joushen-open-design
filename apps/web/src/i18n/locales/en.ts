...
export const en: Dict = {
  ...
  'chat.amrCard.switchBody': 'Switch to Joushen Studio\'s official AMR model service — no API key setup needed. After you sign in, authorize, and top up, this run retries automatically.',
  ...
  'settings.onboardingCreateBody': 'Describe the site, app, deck, image, or video you want. Joushen Studio will create a project and keep the work editable.',
  ...
  'settings.onboardingGateTooltipAmr': 'The next steps run on AI — sign in to Joushen Studio AMR to continue.',
  ...
  'settings.agentInstall.pathHint': 'If you installed a CLI with npm or Homebrew and it still shows as not installed, ensure the tool\'s bin directory is on the PATH the Joushen Studio daemon inherits (Terminal vs GUI apps can differ on macOS). See QUICKSTART.md (section "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Open Install or Docs for your preferred agent.',
  'settings.agentInstall.stepAuth': 'Authenticate with the vendor CLI (sign in or add API credentials) before returning to Joushen Studio.',
  ...
  'settings.mcpServerHint': 'Expose Joushen Studio as an MCP server for your coding agent.',
  ...
  'settings.privacyHint': 'What data is shared with the Joushen Studio team',
  'settings.privacyConsentKicker': 'Help us improve Joushen Studio',
  'settings.privacyConsentLead': 'Joushen Studio can share usage data with our team to help us improve. This includes:',
  ...
  'settings.mcpHint': 'Let coding agents (Cursor, Claude Code, VS Code…) read your Joushen Studio projects directly.',
  'settings.mcpDaemonError': 'Couldn\'t reach the local daemon to resolve install paths ({error}). Make sure Joushen Studio is running, then reopen this panel.',
  'settings.mcpBuildDaemon': 'Build the daemon first.',
  'settings.mcpNodeMissing': 'Node binary is missing.',
  'settings.mcpBuildHint': 'apps/daemon/dist/cli.js is missing. Run `pnpm --filter @joushen-studio/daemon build` and refresh.',
  ...
  'settings.mcpCapabilityDefault': 'Default to the project and file you have open in Joushen Studio, so you can say "build this in my app" without re-stating which design.',
  'settings.mcpRunningNote': 'Joushen Studio must be running for MCP tool calls to succeed. If you started your coding agent before opening Joushen Studio, restart the agent so it can reach the live daemon.',
  ...
  'entry.discordAria': 'Join the Joushen Studio Discord',
  'entry.discordAriaWithOnline': 'Join the Joushen Studio Discord - {online}',
  ...
  'homeHero.chip.createPluginHint': 'Author a reusable Joushen Studio plugin and add it to My plugins.',
  ...
  'handoff.promptIntro': 'Continue from this local Joushen Studio project folder:',
  ...
  'designSystemPicker.noPreview': 'No preview page. Joushen Studio Systems to view the full preview.',
  ...
  'pluginsHome.subtitle': 'Ready-to-use Joushen Studio workflows bundled with this runtime. Pick one to load a starter prompt, or browse the registry for more.',
  'pluginsHome.browseRegistry': 'Browse registry',
  'pluginsHome.count': '{filtered} of {total}',
  'pluginsHome.loadingCatalog': 'Loading catalog…',
  'pluginsHome.emptyCatalog': 'Catalog is empty. Bundled plugins ship with Joushen Studio and should appear here automatically — try restarting the daemon if this persists.',
  ...
  'pluginCard.contributeAria': 'Contribute {title} to Joushen Studio',
  'pluginCard.contributeTitle': 'Contribute plugin to Joushen Studio with a pull request',
  ...
  'integrations.lede': 'Connect external systems, bring MCP tools into your agent loop, and use Joushen Studio from other IDEs, scripts, and automations.',
  ...
  'mcpClient.daemonError': 'Could not reach the local daemon. Make sure Joushen Studio is running, then reopen this panel.',
  'mcpClient.saveFailed': 'Save failed. Check that the daemon is running and try again.',
  'useEverywhere.modalAria': 'Use Joushen Studio everywhere',
  'useEverywhere.modalTitle': 'Use Joushen Studio everywhere',
  'useEverywhere.modalSubtitle': 'Drop Joushen Studio into any IDE, agent, or script — CLI, HTTP, MCP, and Skills. Use “Copy guide for an agent” and paste into Claude Code, Codex, Cursor, openclaw, or hermes to set up everything.',
  ...
  'useEverywhere.section.overview.heading': 'Joushen Studio works wherever your agent works',
  'useEverywhere.section.overview.intro': 'Joushen Studio is more than a window — it is a local privileged daemon (`od`) plus a Skills + Design-Systems + Atoms registry. Once it is running on your machine, any code agent (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, your own script) can drive generations, inspect projects, and produce design artifacts through four interchangeable surfaces.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od <command>` for headless scripts, CI, and shell automation.',
  'useEverywhere.section.overview.bullet2': 'MCP server — wires Joushen Studio as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
  ...
  'useEverywhere.section.overview.bullet4': 'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Joushen Studio at all.',
  ...
  'useEverywhere.section.cli.heading': 'Drive Joushen Studio from any shell',
  ...
  'useEverywhere.section.mcp.heading': 'Expose Joushen Studio as an MCP server to any coding agent',
  'useEverywhere.section.mcp.intro': 'Joushen Studio ships with a Model Context Protocol server (`od mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover Joushen Studio tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
  ...
  'useEverywhere.section.mcp.footer': 'In the Joushen Studio app, open Settings → Integrations to copy a client-specific install command (Cursor, Claude Code, Antigravity, VS Code) instead of editing JSON by hand.',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': 'Same REST + SSE surface the web UI uses',
  'useEverywhere.section.http.intro': 'The local daemon serves an HTTP API at `http://127.0.0.1:7456` (port configurable). Every endpoint the web UI calls is also fair game for your scripts. Streaming endpoints (chat turns, project runs) emit Server-Sent Events with the contract types in `@joushen-studio/contracts`.',
  ...
  'useEverywhere.section.http.footer': 'Pure TypeScript types for every request/response live in `@joushen-studio/contracts` — import them in your script for full autocomplete without wiring a generator.',
  'useEverywhere.section.skills.tab': 'Skills & headless',
  'useEverywhere.section.skills.heading': 'Drop-in Skills for any agent — even without Joushen Studio running',
  'useEverywhere.section.skills.intro': 'A Skill is a directory with a Claude-compatible `SKILL.md` (YAML front-matter + body). Joushen Studio extends the format with the `od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so the same artifact can be used both inside Joushen Studio and by a vanilla agent like Claude Code, Codex, openclaw, or hermes. Discovery follows a precedence chain so projects can override their own skills.',
  ...
  'useEverywhere.section.skills.snippet1': 'Minimal SKILL.md (Claude-compatible front matter + Joushen Studio extras)',
  ...
  'entry.githubStarAria': 'Star Joushen Studio on GitHub',
  ...
  'newproj.locationDefault': 'Joushen Studio projects',
  ...
  'socialShare.openDesignSection': 'Share Joushen Studio',
  'socialShare.openDesignTitle': 'Joushen Studio on GitHub',
  'socialShare.openDesignText': 'Joushen Studio is an open-source workspace for creating, editing, deploying, and handing off design artifacts.',
  ...
  'socialShare.projectText': 'Built with Joushen Studio: {title}. View the deployed artifact and star the repo: {repo}',
  'socialShare.projectCopyText': 'Built with Joushen Studio: {title}\n{url}\nJoushen Studio repo: {repo}',
  ...
  'chat.activeFilePlaceholder': 'Ask Joushen Studio to change {file}...',
  ...
  'chat.mode.design.solves': 'Use it when you want Joushen Studio to generate files, update the workspace, or turn a brief into multimodal deliverables.',
  ...
  'preview.shareTextDefault': 'Joushen Studio template: {title}',
  ...
  'preview.errorBody': 'The example HTML failed to fetch. Make sure Joushen Studio is running and try again.',
  'preview.retry': 'Retry',
  ...
  'designFiles.usefulInfoTip7': 'Enjoying Joushen Studio? Star us on GitHub.',
  ...
  'assistant.shareToJoushenStudio': 'Share to Joushen Studio',
  'assistant.shareToJoushenStudioBusy': 'Preparing package…',
  ...
  'pet.tabBuiltInHint': 'Curated companions bundled with Joushen Studio — pick one and adopt.',
  ...
  'pet.codexBundledTitle': 'Ships with Joushen Studio — no download needed.',
  ...
  'settings.projectLocationsDescription': 'Add work bases that can contain multiple Joushen Studio project folders. New projects are saved as one folder inside the selected base.',
  ...
  'settings.customInstructionsHint': 'Fixed instructions JoushenStudio should follow in every chat. Save facts, preferences, and project context as memories.',
  ...
  'settings.memoryStarterProjectDesc': 'Joushen Studio v0.5 — chat-driven design editor',
  ...
  'updater.availableBody': 'Joushen Studio {version} is available. It will download before the installer can open.',
  ...
  'updater.installerOpenBody': 'The installer is open. Joushen Studio is quitting so you can finish the update.',
  ...
  'updater.quitButton': 'Quit Joushen Studio',
  'updater.quitFailedBody': 'The installer is open, but Joushen Studio could not quit. Quit Joushen Studio before replacing the app.',
  'updater.quitFailedTitle': 'Could not quit',
  'updater.quitting': 'Quitting...',
  'updater.ready': 'Update ready',
  'updater.readyGeneric': 'A new version is ready. Joushen Studio will close and open the installer.',
  'updater.readyVersion': 'Joushen Studio {version} is ready. Joushen Studio will close and open the installer.',
  ...
  'dsManager.createBody': 'Teach Joushen Studio your brand, product, code, assets, and design references.',
  ...
  'settings.customInstructionsDesc': 'Fixed instructions Joushen Studio follows in every chat. These are not saved memories; use Memory for facts, preferences, and project context.',
