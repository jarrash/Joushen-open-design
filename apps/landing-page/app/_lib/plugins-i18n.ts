...
const en: PluginsCopy = {
  ...
  hubLead:
    'Joushen Studio is built around four kinds of plugin. Templates and Skills are what your agent runs; Systems and Craft are how it stays on-brand and accessible. Pick a section to drill in, or jump straight to a slug if you already know which one you want.',
  ...
  systemsMetaTitle: (n) => `Design System Examples — ${n} Open-Source Design Systems | Joushen Studio`,
...
→ ${url}`,
  ...
  faqItems: [
    {
      question: 'What are Joushen Studio templates?',
      answer:
        'Bundled-plugin templates that ship with Joushen Studio — the open source Claude Design alternative. Each one is a runnable artifact: a prototype, slide deck, image generator, video composition, or HyperFrames motion piece. Your local agent runs the plugin against its prompt and an optional example HTML, and produces a ready-to-share asset on your own machine.',
    },
    {
    ...
    },
    {
      ...
      answer:
        "Yes. Joushen Studio is BYOK at every layer — your Claude / OpenAI / local-model credentials never leave your machine. The marketing site doesn't proxy any inference; the live previews you see on the catalogue rows come from posters and Cloudflare Stream URLs the templates ship with, not from a hosted runtime.",
    },
    {
...
const overrides: Partial<Record<LandingLocaleCode, Partial<PluginsCopy>>> = {
  zh: {
    ...
    hubLead:
      'Joushen Studio 围绕四类插件构建：Templates 与 Skills 是 agent 真正运行的内容，Systems 与 Craft 让它保持品牌一致和可访问。点进任意一类深入查看，或直接跳到你已经知道 slug 的那一项。',
    tileTemplates: '模板',
    tileSkills: '技能',
    tileSystems: '设计系统',
    tileCraft: '工艺',
    ...
    systemsMetaTitle: (n) => `设计系统范例 — ${n} 个开源设计系统 | Joushen Studio`,
    systemsMetaDescription:
      '浏览你的 coding agent 可自动套用的设计系统范例——来自真实品牌的色板、字体、动效与文风。开源、BYOK，支持 Claude、Codex、Cursor。',
    systemsAboutHead: '什么是设计系统？',
    systemsAboutBody:
      '设计系统是一套可复用的品牌基础——色板、字体、间距、动效与文风——让每个界面保持一致。在 Joushen Studio 中，每个设计系统都是一个插件：把项目绑到某个系统，你的 coding agent 会自动继承它的色板、字体、动效与文风，产出始终贴合品牌。',
    ...
    faqItems: [
      {
        question: '什么是 Joushen Studio 模板？',
        answer:
          'Joushen Studio 附带的打包插件模板 — Joushen Studio 是开源的 Claude Design 替代品。每个模板都是一个可运行的交付物：原型、幻灯片组、图像生成器、视频合成或 HyperFrames 动画作品。本地 agent 根据提示词和可选的示例 HTML 运行插件，在你的机器上生成可立即分享的资源。',
      },
      {
        question: '模板采用什么许可证？',
        answer:
      ...
      },
      {
        ...
        answer:
          '可以。Joushen Studio 在每个层级都支持 BYOK（自带密钥）— 你的 Claude / OpenAI / 本地模型凭证永远不会离开你的机器。营销网站不会代理任何推理；你在目录行中看到的实时预览来自模板附带的海报和 Cloudflare Stream URL，而不是来自托管的运行时。',
      },
      {
        question: '我如何贡献一个模板？',
        answer:
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio 是 Anthropic 的托管产品。Joushen Studio 是**开源的 Claude Design 替代品** — 此目录中的每个模板、提示词和设计系统都存在于公开仓库中，针对你选择的密钥在本地运行，并可通过任何人都能创作的插件进行扩展。我们采用相同的交付物分类法（原型、幻灯片、图像、视频），便于概念转移，但从 agent 运行时到其他所有内容都保留在你的机器上。',
      },
      {
        question: '预览从哪里来？',
        answer:
  ...
  'zh-tw': {
    ...
    hubLead: 'Joushen Studio 圍繞四類外掛構建：Templates 與 Skills 是 agent 真正執行的內容，Systems 與 Craft 讓它保持品牌一致與可存取性。',
    ...
    systemsMetaTitle: (n) => `設計系統範例 — ${n} 個開源設計系統 | Joushen Studio`,
    ...
    systemsAboutBody: '設計系統是一套可重用的品牌基礎——色票、字體、間距、動效與語氣——讓每個介面保持一致。在 Joushen Studio 中，每個設計系統都是一個外掛：綁定專案到某個系統，你的 coding agent 會自動繼承它的色票、字體、動效與語氣，產出始終貼合品牌。',
    ...
    faqItems: [
      {
        question: 'Joushen Studio 範本是什麼？',
        answer:
          'Joushen Studio 隨附的套裝外掛範本 — 開源 Claude Design 替代方案。每一個都是可執行的成品：原型、簡報、圖像生成器、影片編排或 HyperFrames 動畫作品。你的本機 agent 會針對其提示和可選的範例 HTML 執行外掛，並在你自己的機器上生成可立即分享的資產。',
      },
      {
        question: '範本如何授權？',
        answer:
      ...
      },
      {
        ...
        answer:
          '可以。Joushen Studio 在每一層都支持 BYOK — 你的 Claude / OpenAI / 本機模型憑證永遠不會離開你的機器。行銷網站不會代理任何推理；你在目錄列中看到的即時預覽來自範本隨附的海報和 Cloudflare Stream URL，而非託管執行時環境。',
      },
      {
        question: '我如何貢獻範本？',
        answer:
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio 是 Anthropic 的託管產品。Joushen Studio 是 <strong>開源 Claude Design 替代方案</strong> — 此目錄中的每一個範本、提示和設計系統都存在於公開回購中、針對你選擇的金鑰在本機執行，並可透過任何人都能編寫的外掛進行擴充。我們採用相同的成品分類系統（原型、簡報、圖像、影片），所以心理模型保持一致，但從 agent 執行時環境一切都保留在你的機器上。',
      },
      {
        question: '預覽從何而來？',
        answer:
  ...
  ja: {
    ...
    hubLead: 'Joushen Studio は 4 種類のプラグインを軸に作られています：テンプレートとスキルは agent が実行するもの、システムとクラフトはブランドとアクセシビリティを守るもの。セクションを選んで掘り下げるか、目当ての slug が決まっているなら直接ジャンプしてください。',
    ...
    faqItems: [
      {
        question: 'Joushen Studioのテンプレートとは？',
        answer:
          'Joushen Studioに付属するバンドル型プラグインテンプレートです。Joushen StudioはオープンソースのClaude Design代替ツールです。各テンプレートは実行可能なアーティファクト — プロトタイプ、スライドセット、画像生成器、動画コンポジション、またはHyperFramesモーション作品のいずれか。ローカルエージェントがプロンプトとオプションのサンプルHTMLに対してプラグインを実行し、あなたのマシン上で共有可能なアセットを生成します。',
      },
      {
      ...
      },
      {
        ...
        answer:
          '可能です。Joushen Studioはあらゆる層でBYOK対応 — あなたのClaude / OpenAI / ローカルモデルの認証情報はマシンを離れません。マーケティングサイトは推論をプロキシしません。カタログ行に表示されるライブプレビューはテンプレートに付属するポスターとCloudflare Stream URLから来たもので、ホストされたランタイムからではありません。',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Claude Design StudioはAnthropicのホスト型プロダクトです。Joushen Studioは<strong>オープンソースのClaude Design代替ツール</strong> — このカタログ内のすべてのテンプレート、プロンプト、デザインシステムは公開リポジトリに存在し、あなたが選んだキーでローカルで実行でき、誰でも作成できるプラグインを通じて拡張可能です。同じアーティファクト分類（プロトタイプ、スライド、画像、動画）をミラーしているのでメンタルモデルは引き継がれますが、エージェントランタイムまですべてあなたのマシン上に留まります。',
      },
      {
  ...
  ko: {
    ...
    hubLead: 'Joushen Studio은 네 가지 플러그인 종류를 중심으로 구성됩니다: 템플릿과 스킬은 agent가 실행하는 것, 시스템과 크래프트는 브랜드와 접근성을 지키는 것. 섹션을 골라 들어가거나, 원하는 slug를 알고 있다면 바로 이동하세요.',
    ...
    faqItems: [
      {
        question: 'Joushen Studio 템플릿이란 무엇인가요?',
        answer:
          'Joushen Studio — open source Claude Design 대체 솔루션과 함께 제공되는 번들 플러그인 템플릿입니다. 각각은 실행 가능한 아티팩트입니다: 프로토타입, 슬라이드 덱, 이미지 생성기, 비디오 컴포지션 또는 HyperFrames 모션 작품. 로컬 에이전트가 플러그인을 프롬프트 및 선택적 예제 HTML에 대해 실행하면, 자신의 머신에서 공유할 준비가 된 에셋을 생성합니다.',
      },
      {
      ...
      },
      {
        ...
        answer:
          '네, 가능합니다. Joushen Studio은 모든 계층에서 BYOK를 지원합니다 — Claude / OpenAI / 로컬 모델 인증 정보가 머신을 떠나지 않습니다. 마케팅 사이트는 추론을 프록시하지 않고, 카탈로그 행에서 보는 라이브 미리보기는 호스팅된 런타임이 아니라 템플릿과 함께 제공되는 포스터 및 Cloudflare Stream URL에서 나옵니다.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio는 Anthropic의 호스팅 제품입니다. Joushen Studio은 <strong>open source Claude Design 대체 솔루션</strong>입니다 — 이 카탈로그의 모든 템플릿, 프롬프트, 디자인 시스템은 공개 리포지토리에 있고, 선택한 키에 대해 로컬에서 실행되며, 누구나 작성할 수 있는 플러그인을 통해 확장할 수 있습니다. 동일한 아티팩트 분류(프로토타입, 슬라이드, 이미지, 비디오)를 반영하므로 멘탈 모델이 유지되지만, 에이전트 런타임까지 모든 것이 머신에 머물러 있습니다.',
      },
      {
  ...
  de: {
    ...
    hubLead: 'Joushen Studio ist um vier Plugin-Arten herum gebaut: Templates und Skills sind das, was dein Agent ausführt; Systems und Craft halten Marke und Zugänglichkeit. Wähle eine Sektion zum Vertiefen oder springe direkt zu einem Slug, wenn du schon weißt, welches du willst.',
    ...
    faqItems: [
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio ist Anthropics gehostetes Produkt. Joushen Studio ist die <strong>open source Claude Design Alternative</strong> — jedes Template, jeder Prompt und jedes Design System in diesem Katalog lebt in einem öffentlichen Repo, läuft lokal gegen die Keys deiner Wahl und kann durch Plugins erweitert werden, die jeder verfassen kann. Wir verwenden die gleiche Artefakt-Taxonomie (Prototypen, Slides, Bilder, Video), damit die mentale Modellierung übertragbar ist, aber alles bis hinunter zur Agent-Runtime bleibt auf deinem Rechner.',
      },
      {
  ...
  fr: {
    ...
    hubLead: 'Joushen Studio s’articule autour de quatre types de plugins : Templates et Skills sont ce que ton agent exécute ; Systems et Craft maintiennent la marque et l’accessibilité. Choisis une section ou saute directement à un slug si tu sais déjà ce que tu veux.',
    ...
    faqItems: [
      {
        question: "Qu'est-ce que les modèles Joushen Studio ?",
        answer:
          "Des modèles avec plugins intégrés qui sont livrés avec Joushen Studio — l'alternative open source à Claude Design. Chacun est un artefact exécutable : un prototype, un diaporama, un générateur d'images, une composition vidéo, ou une pièce de mouvement HyperFrames. Votre agent local exécute le plugin selon son prompt et un HTML d'exemple optionnel, et produit un atout prêt à partager sur votre propre machine.",
      },
      {
  ...
  ru: {
    ...
    hubLead: 'Joushen Studio построен вокруг четырёх видов плагинов: Templates и Skills — это то, что выполняет твой agent; Systems и Craft удерживают бренд и доступность. Выбери раздел для углубления или сразу перейди по slug, если уже знаешь, что нужно.',
    ...
    faqItems: [
      {
        question: 'Что такое шаблоны Joushen Studio?',
        answer:
          'Шаблоны с встроенными плагинами, которые поставляются с Joushen Studio — открытую альтернативу Claude Design. Каждый из них — это исполняемый артефакт: прототип, слайд-дек, генератор изображений, видеокомпозиция или motion-piece на HyperFrames. Локальный агент запускает плагин по его промпту и опциональному примеру HTML, и создаёт готовый к публикации ассет на вашей машине.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Да. Joushen Studio работает с собственными ключами на каждом уровне — ваши учётные данные Claude / OpenAI / локальной модели никогда не покидают машину. Сайт маркетинга не проксирует никакое inference; live-превью, которые вы видите в строках каталога, поступают из постеров и Cloudflare Stream URL, которые поставляются с шаблонами, а не из размещённого runtime.',
      },
      {
  ...
  es: {
    ...
    hubLead: 'Joushen Studio se construye alrededor de cuatro tipos de plugin: Templates y Skills son lo que tu agent ejecuta; Systems y Craft mantienen marca y accesibilidad. Elige una sección para profundizar o salta directo a un slug si ya sabes cuál quieres.',
    ...
    faqItems: [
      {
        question: '¿Qué son las plantillas de Joushen Studio?',
        answer:
          'Plantillas de plugins integrados que se incluyen con Joushen Studio — la alternativa open source a Claude Design. Cada una es un artefacto ejecutable: un prototipo, un conjunto de diapositivas, un generador de imágenes, una composición de vídeo o una pieza de movimiento HyperFrames. Tu agente local ejecuta el plugin contra su prompt y un HTML de ejemplo opcional, y produce un activo listo para compartir en tu máquina.',
      },
      {
  ...
  'pt-br': {
    ...
    hubLead: 'Joushen Studio é construído em torno de quatro tipos de plugin: Templates e Skills são o que seu agent executa; Systems e Craft mantêm marca e acessibilidade. Escolha uma seção para se aprofundar ou pule direto para um slug se já sabe o que quer.',
    ...
    faqItems: [
      {
        question: 'O que são os templates Joushen Studio?',
        answer:
          'Templates com plugin integrado que vêm com Joushen Studio — a alternativa open source para Claude Design. Cada um é um artefato executável: um protótipo, deck de apresentação, gerador de imagem, composição de vídeo ou uma peça de motion HyperFrames. Seu agente local executa o plugin contra seu prompt e um HTML de exemplo opcional, gerando um ativo pronto para compartilhar na sua máquina.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Sim. Joushen Studio é BYOK em todas as camadas — suas credenciais Claude / OpenAI / modelo local nunca saem da sua máquina. O site de marketing não faz proxy de nenhuma inferência; as visualizações ao vivo que você vê nas linhas do catálogo vêm de cartazes e URLs do Cloudflare Stream que os templates incluem, não de um runtime hospedado.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio é o produto hospedado da Anthropic. Joushen Studio é a <strong>alternativa open source para Claude Design</strong> — cada template, prompt e design system neste catálogo reside em um repositório público, executa localmente contra as chaves que você escolhe e pode ser estendido através de plugins que qualquer pessoa pode criar. Espelhamos a mesma taxonomia de artefatos (protótipos, apresentações, imagens, vídeo) para que o modelo mental seja transferível, mas tudo, até o runtime do agente, fica na sua máquina.',
      },
      {
  ...
  it: {
    ...
    hubLead: 'Joushen Studio è costruito attorno a quattro tipi di plugin: Templates e Skill sono ciò che il tuo agent esegue; Systems e Craft tengono insieme brand e accessibilità. Scegli una sezione in cui scendere o salta a uno slug se sai già quale ti serve.',
    ...
    faqItems: [
      {
        question: 'Cosa sono i template di Joushen Studio?',
        answer:
          "Template con plugin inclusi che vengono forniti con Joushen Studio — l'alternativa open source a Claude Design. Ogni template è un artefatto eseguibile: un prototipo, un mazzo di slide, un generatore di immagini, una composizione video o un pezzo di motion di HyperFrames. L'agente locale esegue il plugin rispetto al suo prompt e a un HTML di esempio opzionale, e produce un asset pronto da condividere sul tuo computer.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Claude Design Studio è il prodotto hosted di Anthropic. Joushen Studio è l'<strong>alternativa open source a Claude Design</strong> — ogni template, prompt e design system in questo catalogo vive in un repo pubblico, viene eseguito localmente rispetto alle chiavi che scegli, e può essere esteso attraverso plugin che chiunque può creare. Rispecchiamo la stessa tassonomia di artefatti (prototipi, slide, immagini, video) così il modello mentale rimane lo stesso, ma tutto fino al runtime dell'agente rimane sulla tua macchina.",
      },
      {
  ...
  id: {
    ...
    hubLead: 'Joushen Studio dibangun di sekitar empat jenis plugin: Templates dan Skills adalah yang dijalankan agent kamu; Systems dan Craft menjaga brand dan aksesibilitas. Pilih satu bagian untuk mendalami, atau lompat langsung ke slug kalau sudah tahu mau yang mana.',
    ...
    faqItems: [
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio adalah produk hosted Anthropic. Joushen Studio adalah <strong>alternatif open source Claude Design</strong> — setiap template, prompt, dan design system di katalog ini tinggal di repo publik, berjalan lokal terhadap kunci yang Anda pilih, dan dapat diperluas melalui plugin yang siapa pun bisa buat. Kami mencerminkan taksonomi artefak yang sama (prototipe, slide, gambar, video) sehingga model mental terbawa, tetapi semuanya hingga runtime agen tetap di mesin Anda.',
      },
      {
  ...
  pl: {
    ...
    hubLead: 'Joushen Studio opiera się na czterech rodzajach wtyczek: Templates i Skills to to, co uruchamia twój agent; Systems i Craft pilnują marki i dostępności. Wybierz sekcję, by zejść głębiej, albo skocz prosto do sluga, jeśli już wiesz, czego chcesz.',
    ...
    faqItems: [
      {
        question: 'Czym są szablony Joushen Studio?',
        answer:
          "Szablony z wbudowanymi wtyczkami, które dostarczane są z Joushen Studio — open source'ową alternatywą Claude Design. Każdy to artefakt, który można uruchomić: prototyp, prezentacja, generator obrazów, kompozycja wideo lub piece motion HyperFrames. Twój lokalny agent uruchamia wtyczkę względem jej promptu i opcjonalnego przykładowego HTML-a, produkcując gotowy do udostępnienia zasób na Twojej maszynie.",
      },
      {
      ...
      },
      {
        ...
        answer:
          'Tak. Joushen Studio to BYOK na każdej warstwie — Twoje dane uwierzytelniające Claude / OpenAI / lokalnego modelu nigdy nie opuszczają Twoją maszynę. Witryna marketingowa nie realizuje żadnych wnioskodawań; podglądy na żywo, które widzisz na rzędach katalogów, pochodzą z plakatów i adresów URL Cloudflare Stream, które dostarczają szablony, a nie z hostowanego środowiska uruchomieniowego.',
      },
      {
  ...
  ar: {
    ...
    hubLead: 'تم بناء Joushen Studio حول أربعة أنواع من الإضافات: القوالب والمهارات هي ما يشغّله الـ agent؛ والأنظمة والحرفة تحافظان على الهوية وإمكانية الوصول. اختر قسما للتعمق، أو انتقل مباشرة إلى slug إذا كنت تعرف ما تريد.',
    ...
    faqItems: [
      {
        question: 'ما هي قوالب Joushen Studio؟',
        answer:
          'قوالب مدمجة تأتي مع Joushen Studio — البديل مفتوح المصدر لـ Claude Design. كل واحد منها أداة قابلة للتشغيل: نموذج أولي أو مجموعة شرائح أو مولد صور أو مركبة فيديو أو قطعة حركة HyperFrames. يقوم وكيلك المحلي بتشغيل المكون على أساس أمره وملف HTML توضيحي اختياري، وينتج أصل جاهز للمشاركة على جهازك.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'نعم. Joushen Studio هو BYOK في كل طبقة — بيانات اعتماد Claude و OpenAI والنموذج المحلي الخاصة بك لا تترك جهازك أبداً. موقع التسويق لا يوجه أي استدلال؛ المعاينات المباشرة التي تراها في صفوف الكتالوج تأتي من الملصقات وعناوين URL لـ Cloudflare Stream التي يأتي معها القالب، وليس من وقت تشغيل مستضاف.',
      },
      {
  ...
  tr: {
    ...
    hubLead: 'Joushen Studio dört eklenti türü etrafında inşa edilmiştir: Templates ve Skills agent’ın çalıştırdığı, Systems ve Craft ise marka ve erişilebilirliği koruyan parçalar. Bir bölüme dal ya da hangi slug’u istediğini biliyorsan doğrudan oraya zıpla.',
    ...
    faqItems: [
      {
        question: 'Joushen Studio şablonları nedir?',
        answer:
          "Joushen Studio ile birlikte gelen eklenti paketlenmiş şablonlar — açık kaynak Claude Design alternatifi. Her biri çalıştırılabilir bir artifact: prototip, slayt seti, resim oluşturucu, video композисі veya HyperFrames hareket parçası. Yerel aracınız eklentiyi prompt ve isteğe bağlı örnek HTML'e karşı çalıştırır ve makinenizde paylaşılmaya hazır bir asset üretir.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Evet. Joushen Studio her katmanda BYOK'dur — Claude / OpenAI / yerel model kimlik bilgileriniz makinenizi asla terk etmez. Pazarlama sitesi herhangi bir inference proxy'si değildir; katalog satırlarında gördüğünüz canlı önizlemeler posterlerden ve şablonların sağladığı Cloudflare Stream URL'lerinden gelir, barındırılan bir runtime'dan değil.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Claude Design Studio, Anthropic'in barındırılan ürünüdür. Joushen Studio, <strong>açık kaynak Claude Design alternatifidir</strong> — bu katalogdaki her şablon, prompt ve tasarım sistemi genel bir repo'da yaşar, seçtiğiniz anahtarlara karşı yerel olarak çalışır ve herkesin yazabileceği eklentiler aracılığıyla genişletilebilir. Aynı artifact taksonomisini (prototipler, slaytlar, resimler, video) yansıtıyoruz, böylece zihinsel model devam eder, ancak aracı runtime'a kadar her şey makinenizde kalır.",
      },
      {
  ...
  uk: {
    ...
    hubLead: 'Joushen Studio побудовано навколо чотирьох видів плагінів: Templates і Skills — те, що виконує твій agent; Systems і Craft утримують бренд і доступність. Обери розділ для занурення або одразу перейди по slug, якщо вже знаєш, що потрібно.',
    ...
    faqItems: [
      ...
      },
      {
        ...
        answer:
          'Так. Joushen Studio використовує BYOK на кожному рівні — ваші облікові дані Claude / OpenAI / локальної моделі ніколи не залишають вашу машину. Маркетинговий сайт не проксує жодних висновків; живі попередження, які ви бачите в рядах каталогу, походять від постерів і URL-адрес Cloudflare Stream, які поставляються з шаблонами, а не від розміщеного середовища виконання.',
      },
      {
      ...
      },
      {
        ...
        answer:
          'Claude Design Studio — це розміщений продукт Anthropic. Joushen Studio — це <strong>відкритий вихідний код Claude Design альтернатива</strong> — кожен шаблон, промпт і система дизайну в цьому каталозі живуть у публічному репозиторії, запускаються локально проти обраних вами ключів і можуть розширюватися через плагіни, які може написати будь-хто. Ми дзеркалимо ту ж таксономію артефактів (прототипи, слайди, зображення, відео), щоб ментальна модель перенеслася, але все — від агента виконання до агента — залишається на вашій машині.',
      },
      {
  ...
  vi: {
    ...
    hubLead: 'Joushen Studio xoay quanh bốn loại plugin: Templates và Skills là thứ agent của bạn chạy; Systems và Craft giữ thương hiệu và tính tiếp cận. Chọn một mục để đào sâu, hoặc nhảy thẳng tới một slug nếu bạn đã biết cần gì.',
    ...
    faqItems: [
      {
        question: 'Mẫu Joushen Studio là gì?',
        answer:
          'Các mẫu plugin đi kèm với Joushen Studio — phiên bản alternative open source của Claude Design. Mỗi mẫu là một artifact có thể chạy được: một prototype, bộ slide, trình tạo ảnh, video composition, hoặc một motion piece HyperFrames. Agent cục bộ của bạn chạy plugin với prompt và một HTML ví dụ tùy chọn, và tạo ra một asset sẵn sàng chia sẻ trên máy của bạn.',
      },
      {
  ...
  nl: {
    ...
    hubLead: 'Joushen Studio is gebouwd rond vier soorten plug-ins: Templates en Skills zijn wat je agent draait; Systems en Craft houden merk en toegankelijkheid op orde. Kies een sectie om dieper in te duiken, of spring direct naar een slug als je al weet wat je wilt.',
    ...
    faqItems: [
      {
        question: 'Wat zijn Joushen Studio-sjablonen?',
        answer:
          'Gebundelde plugin-sjablonen die bij Joushen Studio horen — het open source Claude Design-alternatief. Elk sjabloon is een runnable artifact: een prototype, presentatiedeck, imagegenerator, videocompositie of HyperFrames motion piece. Je lokale agent voert de plugin uit tegen zijn prompt en optionele voorbeeld-HTML, en produceert een klaar-om-te-delen asset op je eigen machine.',
      },
      {
      ...
      },
      {
        ...
        answer:
          "Ja. Joushen Studio is BYOK op elk niveau — je Claude / OpenAI / lokale-model-referenties verlaten je machine nooit. De marketingsite proxy geen inference; de live voorbeelden die je in de catalogusrijen ziet, komen van posters en Cloudflare Stream URL's die de sjablonen meebrengen, niet van een gehoste runtime.",
      },
      {
